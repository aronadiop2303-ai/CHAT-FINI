...
export class NeonMessageStore implements MessageStore {
  ...
  async createMessageAndEvent(input: {
    ...
    return this.runner.transaction(async (client) => {
      const existing = input.clientMessageId
        ? await client.query<PersistedMessage & { event_id: string }>(
            `SELECT m.id, m.conversation_id AS "conversationId", m.sender_id AS "senderId",
                    m.kind, m.body, m.created_at AS "createdAt", m.client_message_id AS "clientMessageId",
                    e.id AS event_id
             FROM chat_fini.messages m
             LEFT JOIN chat_fini.events e
               ON e.aggregate_type = 'message' AND e.aggregate_id = m.id AND e.event_type = 'message.created'
             WHERE m.sender_id = $1::uuid AND m.client_message_id = $2
             LIMIT 1`,
      ...
      const inserted = await client.query<PersistedMessage>(
        `WITH authorized AS (
           SELECT 1 FROM chat_fini.conversation_members
         ...
         RETURNING id, conversation_id AS "conversationId", sender_id AS "senderId",
                   kind, body, created_at AS "createdAt", client_message_id AS "clientMessageId"`,
      ...
      const event = await client.query<{ id: string }>(
        `INSERT INTO chat_fini.events
           (event_type, aggregate_type, aggregate_id, actor_id, payload, occurred_at)
         VALUES
           ('message.created', 'message', $1::uuid, $2::uuid,
            jsonb_build_object(
              'id', $1::uuid,
              'conversationId', $3::uuid,
              'senderId', $2::uuid,
              'kind', $4::text,
              'body', $5::text,
              'clientMessageId', $6::text,
      ...
      return { message, eventId: event.rows[0].id };
    });
  }
}
